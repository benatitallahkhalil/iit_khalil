...

{
...

    {
        ...
        try{
            $article=Article::findOrFail($id);
            $article->delete();
            return response()->json("article supprime avec Sucees",200);

        }  catch(\Exception $e){
            return response()->json($e->getMessage(),$e->getCode());
        }     //
    }
    public function showArticlesBySCAT($idcat){
        try{
            $article=Article::where('scategorieID',$idcat)->with('scategorie')->get();
